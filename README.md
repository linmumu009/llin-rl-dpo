...
- `v0.1.1`：初始化并推送 GitHub 仓库；创建 `llin-rl-dpo` 安全容器；完成 MindSpeed-RL/MindSpeed-LLM 容器 smoke test，记录 NPU device_count 仍为 0 的阻塞点。
...
`llin-rl-dpo` 容器已创建并运行，但容器内 `torch_npu.npu.device_count()` 当前返回 `0`。

已试过：

- MindSpeed-RL 2.2.0 A3 ARM 镜像。
- MindSpeed-LLM 26.0.0 A3 镜像。
- 单设备 `/dev/davinci0` 暴露。
- 全设备 `/dev/davinci0-15` 暴露。
- 挂载宿主机 driver/add-ons。
- 对 MindSpeed-RL 镜像补充 CANN 9.0.0 / ascend-toolkit 路径。

`--privileged` 容器方案因为会显著扩大宿主机访问权限，未在没有明确风险确认的情况下执行。
