...
|---|---|---:|---|---|---|
| ModelScope ms-swift | 阿里 ModelScope 框架；官方文档覆盖 Qwen3.6/Qwen3.5、DPO、人类对齐、Ascend NPU、FSDP/FSDP2/DeepSpeed/Megatron | 已跑通：Qwen3.6-27B + DPO + LoRA + FSDP2 + 8 NPU 完成 1/20/100 step；FSDP2 checkpoint 保存通过，sharded resume 仍失败；`FULL_STATE_DICT + save_only_model` 已导出普通 LoRA adapter，并完成 adapter 重新加载和固定 prompts 对照推理 | 100 step 半真实 DPO：`train_runtime=344.9065s`，`train_steps_per_second=0.29`，`train_samples_per_second=2.319`，显存记录 `52.56 GiB`；100-step adapter 固定 prompts 推理 `3.0334 tokens/s`，同轮 base `3.0675 tokens/s` | 100 step 半真实数据 `train_loss=0.01584221`，最终 reward accuracy `1.0`、margin `18.0`；adapter safetensors 可读取，包含 992 个 LoRA tensor；固定 prompts 上 base/adapter 输出完全一致，暂不能证明泛化效果；`enable_thinking=false` 仍保留空 `<think></think>` 前缀 | sharded resume 先后遇到 `assign` 签名和缺视觉塔完整权重问题；真实数据、AICore 利用率、`chunk_gated_delta_rule` warning 影响和 deepspeed 对照仍待测；半真实模板数据容易拟合，效果评估仍不足 |
