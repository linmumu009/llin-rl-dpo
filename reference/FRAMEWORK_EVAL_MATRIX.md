...
|---|---|---:|---|---|---|
| MindSpeed-RL DPO | 服务器已有官方 RL 镜像；官方支持 DPO，但样例是 Qwen3-30B-A3B | 未测试 | 待测 samples/s、step time、HBM、AICore | 待测 DPO loss、reward margin、偏好准确率 | 目标 qwen3.6-27B 是 `qwen3_5` conditional/multimodal 配置，可能不被现成 qwen3 模板支持 |
| MindSpeed-LLM posttrain/DPO | 服务器已有 MindSpeed-LLM 镜像；可能用于权重转换或训练后处理 | 未测试 | 待测 | 待测 | 需要确认是否支持 qwen3_5 权重转换和 DPO 入口 |
