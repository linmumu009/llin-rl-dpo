...
|---|---|---:|---|---|---|
| ModelScope ms-swift | 阿里 ModelScope 框架；官方文档覆盖 Qwen3.6/Qwen3.5、DPO、人类对齐、Ascend NPU、FSDP/FSDP2/DeepSpeed/Megatron | 已跑通：Qwen3.6-27B + DPO + LoRA + FSDP2 + 8 NPU 完成 1 step 和 20 step；FSDP2 checkpoint 保存通过，resume 失败 | 20 step 合成数据：平均约 5.92s/step；保存版 20 step：平均约 4.89s/step，samples/s 1.636，显存记录 51.85 GiB | 合成数据 loss 下降到 train_loss=0.0722，未做真实验证集 | FSDP2 checkpoint resume 失败：`SwiftModel.load_state_dict(assign=...)` 不兼容；真实数据、AICore 利用率和 deepspeed 对照仍待测 |
