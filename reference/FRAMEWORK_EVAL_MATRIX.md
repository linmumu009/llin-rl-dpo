...
|---|---|---:|---|---|---|
| ModelScope ms-swift | 阿里 ModelScope 框架；官方文档覆盖 Qwen3.6/Qwen3.5、DPO、人类对齐、Ascend NPU、FSDP/FSDP2/DeepSpeed/Megatron | 已跑通：Qwen3.6-27B + DPO + LoRA + FSDP2 + 8 NPU 完成 1 step 和 20 step；FSDP2 checkpoint 保存通过，sharded resume 仍失败；`FULL_STATE_DICT + save_only_model` 已导出普通 LoRA adapter，并完成 adapter 重新加载推理 smoke test | 20 step 合成数据：平均约 5.92s/step；保存版 20 step：平均约 4.89s/step，samples/s 1.636，显存记录 51.85 GiB；adapter 推理 smoke 为 16 generated tokens / 49.2921s，tokens/s 0.3246 | 合成数据 loss 下降到 train_loss=0.0722，未做真实验证集；adapter safetensors 可读取，包含 992 个 LoRA tensor；adapter 可生成但 smoke 输出被截断，不代表效果 | sharded resume 先后遇到 `assign` 签名和缺视觉塔完整权重问题；真实数据、AICore 利用率、`chunk_gated_delta_rule` warning 影响和 deepspeed 对照仍待测 |
